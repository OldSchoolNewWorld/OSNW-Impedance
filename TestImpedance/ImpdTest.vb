...
Namespace DevelopmentTests
    ...
    Public Class TryMatchArbitraryTests
        ...
        '    Dim DiagInfoString As New System.Text.StringBuilder
        '    DiagInfoString.Append($"{NameOf(Y0)}: {Y0.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(LoadR)}: {LoadR.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(LoadX)}: {LoadX.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(LoadCircR)}: {LoadCircR.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(LoadY)}: {LoadY.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(LoadG)}: {LoadG.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(LoadB)}: {LoadB.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(LoadCircG)}: {LoadCircG.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(TargetR)}: {TargetR.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(TargetX)}: {TargetX.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(TargetCircR)}: {TargetCircR.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(TargetY)}: {TargetY.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(TargetG)}: {TargetG.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(TargetB)}: {TargetB.ToString()}")
        '    DiagInfoString.Append($"; {NameOf(TargetCircG)}: {TargetCircG.ToString()}")


        '    'DiagInfoString = {Y0:1; LoadR:1; LoadX:1; LoadCircR:Center: (5, 5), Radius: 1; LoadY:<0.5; -0.5>;
        '    'LoadG:0.5; LoadB:-0.5; LoadCircG:Center: (3.333333333333333, 5), Radius: 1.3333333333333333;
        '    'TargetR:2; TargetX:-2; TargetCircR:Center: (5.333333333333333, 5), Radius: 0.666666...



        '    ' Move
        '    '     CW or CCW on the LoadG circle to the TargetR circle. There may
        '    '     be either one or two intersections.
        '    '     Then CW or CCW on the TargetR circle to TargetZ.
        '    ' or
        '    ' Move
        '    '     CW or CCW on the LoadR circle to the TargetG circle. There may
        '    '     be either one or two intersections.
        '    '     Then CW or CCW on the TargetG circle to TargetZ.

        '    ' DO THIS FOR BOTH OF THE CASES LISTED JUST ABOVE.
        '    ' Determine the circle intersections.
        '    Dim Intersections _
        '        As System.Collections.Generic.List(Of OSNW.Numerics.PointD) =
        '             GenericCircle.GetIntersections(LoadCircR, TargetCircG)
        ...
        '    ' There are now be either one or two intersection points. With two,
        '    ' there is be one above, and one below, the resonance line; the X
        ...
        '        MainCirc, targetZ, Transformation1) Then
        '            Return False
        '        End If
