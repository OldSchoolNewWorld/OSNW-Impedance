...
Namespace MatchArbitraryTests
    ...
    Public Class TestMatchArbitraryZ0
        ...
        Public Sub MatchArbitrary_Z0_Succeeds(z0 As System.Double, loadR As Double, loadX As Double)
        ...
        Public Sub MatchArbitraryZ0_GoodInput_Succeeds(z0 As System.Double, loadR As Double, loadX As Double,
            ...
            Impedance.MatchArbitrary(z0, LoadZ, SourceZ, Transformations)
            ...
            Dim RowStr As New StringBuilder(1000)
            Dim OutStr As String

            OutStr = String.Format("{0}", name)
            RowStr.AppendLine(OutStr)

            OutStr = String.Format("Z0={0}, LoadZ={1}", z0, LoadZ.ToString)
            RowStr.AppendLine(OutStr)

            If Transformations.Length.Equals(0) Then
                OutStr = "No solution."
                RowStr.AppendLine(OutStr)
            Else
                For Each OneTrans As Transformation In Transformations
                    OutStr = String.Format("{0}, Values={1}; {2}; {3}", OneTrans.Style, OneTrans.Value1, OneTrans.Value2, OneTrans.Value3)
                    RowStr.AppendLine(OutStr)
                Next
            End If
            RowStr.AppendLine(String.Empty)

            System.Diagnostics.Debug.Write($"{RowStr}")
    ...
    Public Class TestMatchArbitraryAny
        ...
        Public Sub MatchArbitrary_Succeeds(z0 As Double, loadR As Double, loadX As Double, sourceR As Double,
                                           sourceX As Double, name As String)
