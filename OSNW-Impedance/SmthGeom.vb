...
Public Class GenericCircle
    ...
    Public Shared Function GetIntersections(ByVal c1X As System.Double,
        ...
        ' There will now be one or two intersections.
        Dim Intersections _
            As New System.Collections.Generic.List(Of OSNW.Numerics.PointD)

        If Impedance.EqualEnough(c1X, c2X) AndAlso
            Impedance.EqualEnough(c1Y, c2Y) AndAlso
            Impedance.EqualEnough(c1R, c2R) Then

            ' Same circle, infinite intersection points.
        ...
        ' ARE SPECIAL CASES NEEDED FOR TANGENT CIRCLES STACKED VERTICALLY,
        ' INFINITE TAN(), OR HORIZONTALLY?
        ...
        If Impedance.EqualEnough(Math.Abs(c1R - c2R), DeltaCtr) Then
            ...
            'If c1R > c2R Then
            '    Dim Factor As System.Double = c1R / (c1R - c2R)
            '    Dim OffsetX As System.Double = Factor * DeltaX
            '    Dim OffsetY As System.Double = Factor * DeltaY
            '    Dim Isect1 As New OSNW.Numerics.PointD(c1X + OffsetX,
            '                                           c1Y + OffsetY)
            '    Intersections.Add(Isect1)
            '    Return Intersections
            'Else
            '    Dim AddLen As System.Double = c1R
            '    Dim NewLen As System.Double = c2R
            '    Dim Factor As System.Double = NewLen / DeltaCtr
            '    Dim OffsetX As System.Double = Factor * DeltaX
            '    Dim OffsetY As System.Double = Factor * DeltaY
            '    Dim Isect1 As New OSNW.Numerics.PointD(c2X + OffsetX,
            '                                           c2Y + OffsetY)
            '    Intersections.Add(Isect1)
            '    Return Intersections
            'End If
...
''' point, on a grid that contains a <see cref="SmithMainCircle"/>.
''' </summary>
Public Class PlotDetails
    ...
    ''' <summary>
    ''' Returns the voltage reflection coefficient for the impedance represented
    ...
    ''' <summary>
    ''' Returns the power reflection coefficient for the impedance represented
    ''' by the grid location.
    ''' </summary>
    ...
    ''' <summary>
    ''' Returns the voltage standing wave ratio for the impedance represented by
    ''' the grid location.
    ''' </summary>
    ...
    ''' <summary>
    ''' Returns the angle of reflection, in degrees, for the impedance
    ''' represented by the grid location.
    ''' </summary>
    ...
    ''' <summary>
    ''' Returns the angle of transmission, in degrees, for the impedance
    ''' represented by the grid location.
    ''' </summary>
    ...
    ''' <summary>
    ''' Creates a new instance of the <c>PlotDetails</c> class with the
